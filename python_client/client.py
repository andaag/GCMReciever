...
if os.path.exists("python-gcm"):
	sys.path.insert(0, "python-gcm")
...
def createMsg(title, message=None, delay_while_idle=False, expires=None, icon=None, iconBackground=None, collapseKey=None, notification=None, intent=None):
...
msg = createMsg("This is a test 2!", message="This is much more detailed info", 
	...
	iconBackground="red",   #Supported : #RRGGBB #AARRGGBB 'red', 'blue', 'green', 'black', 'white', 'gray', 'cyan', 'magenta', 'yellow', 'lightgray', 'darkgray'
	collapseKey="onlyShowThisTypeOnceInList",  #only one of these events will show up in the app
	notification=createNotification(
		notificationKey="onlyShowThisPopupOnce", #only one of these notifications will be shown, can be shared but don't have to be, note that the key in use will be this.hashCode()
		progress=10, #in %, if 0 considered indeterminate progress
		vibrate=True, 
		sound=False, 
		priority=2 #min = -2, -1 = low prio, 0 = default, anything above = higher. See http://developer.android.com/reference/android/app/Notification.html
		),
	intent={
		"type":"plain/text",
...
def sendMsg(msg, verbose=False):
	...
	gcm.json_request(config["REGIDS"], data=package, collapse_key=collapse_key, time_to_live=time_to_live, delay_while_idle=delay_while_idle)
